...
void PostErrorIfRunning(ERROR_MESSAGE_NUM err)
{
   if (Param::GetInt(Param::opmode) == MOD_RUN)
   {
	  ...
	  Param::SetInt(Param::currenterror, err);
   }
}
...
float GetUserThrottleCommand()
{
   ...
   Param::SetInt(Param::currenterror, ERR_NONE);
   ...
   else if(potmode == POTMODE_DUALCHANNEL)
   {
      ...
      if(inRange1 && inRange2)
      {
         ...
         if(ABS(pot2nomTmp - pot1nomTmp) > 10.0f) // check if more than 10% differance
         {
            utils::PostErrorIfRunning(ERR_THROTTLE12DIFF);
			Param::SetInt(Param::limpmode, 1); // this ensures we stay in limp mode if diff error occurs at any part of throttle range
		 if(pot1nomTmp < pot2nomTmp) useChannel = 0;
		 if(pot2nomTmp < pot1nomTmp) useChannel = 1;
		 }
		...
		// {
         ...
         //   {
        //       if(pot1nomTmp > 50.0f)
         //         pot1val = ABS(Throttle::potmax[0] - Throttle::potmin[0]) / 2; // potval set at mid-range, ABS allows for normal and inverted pot.
        ...
        //    }
         ...
         //   {
         //      if(pot2nomTmp > 50.0f)
         //         pot2val = ABS(Throttle::potmax[1] - Throttle::potmin[1]) / 2;
   ...
   /*if (useChannel == 0)
   {      
      if (Param::Get(Param::limpmode) == 0)
	  {
         return Throttle::CalcThrottle(pot1val, 0, brake);
      }
      else if (Param::Get(Param::limpmode) == 1)
      {
	     return Throttle::CalcThrottle(pot1val, 0, brake)/2;
      }
   }
   else if(useChannel == 1)
   {
	  ...
	  {
         return Throttle::CalcThrottle(pot2val, 1, brake)/2;
	  }
   } 
   else
   return 0.0; */
...

{
   ...
   int uauxGain = 242; // calibrated against DVM
...
float ProcessThrottle(int speed)
{
   ...
   if (Throttle::TemperatureDerate(Param::Get(Param::tempmotor), Param::Get(Param::tmpmmax), finalSpnt))
   {
