...
void nvic_setup(void)
{
   ...
   //nvic_set_priority(NVIC_USB_LP_CAN_RX0_IRQ, 0xe0); //second lowest priority

   //nvic_enable_irq(NVIC_USB_HP_CAN_TX_IRQ); //CAN TX
   //nvic_set_priority(NVIC_USB_HP_CAN_TX_IRQ, 0xe0); //second lowest priority
...
void tim3_setup()
{
     // gpio_set_mode(GPIOC,GPIO_MODE_OUTPUT_2_MHZ,	// Low speed
	//	  GPIO_CNF_OUTPUT_ALTFN_PUSHPULL,GPIO8);	// GPIOC8=TIM3.CH3 ( 0n GP_OUT1 ) with TIM3 remap
		gpio_set_mode(GPIOA,GPIO_MODE_OUTPUT_2_MHZ,	// Low speed
		  GPIO_CNF_OUTPUT_ALTFN_PUSHPULL,GPIO6);	// GPIOA6=TIM3.CH3 ( 0n GP_OUT1 )
  ...
  // timer_set_oc_polarity_high(TIM3, TIM_OC2);
   ...
   timer_set_period(TIM3, 35999); //default to 1 kHz
   timer_set_oc_value(TIM3, TIM_OC3, 18000); //50%
   timer_generate_event(TIM3, TIM_EGR_UG);
   timer_set_prescaler(TIM3, 1); // /2, 36MHz base speed
...
/*    timer_disable_counter(TIM3);
   ...
   timer_set_period(TIM3, 2100);
   timer_set_oc_value(TIM3, TIM_OC3, 800);//duty. 1000 = 52% , 500 = 76% , 1500=28%
   timer_enable_counter(TIM3); */
