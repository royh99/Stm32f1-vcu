 ...
 *
 * @return float Throttle percentage in the range of [0, 100.0]
 */
...
float ProcessThrottle(int speed)
{
    ...
    if (Throttle::TemperatureDerate(Param::GetFloat(Param::tmphs), Param::GetFloat(Param::temphsmax), finalSpnt))
    {
        ErrorMessage::Post(ERR_TMPHSMAX);
    }

    if (Throttle::TemperatureDerate(Param::GetFloat(Param::tempmotor), Param::GetFloat(Param::tempmmax), finalSpnt))
    {
